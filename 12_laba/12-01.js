...
requestController.on('POST', (req, res) => {
    ...
    if (pathName == '/') {
        ...
        let fname = currentDate.getFullYear().toString() + ('0' + currentDate.getMonth() + 1).slice(-2) +
            ('0' + currentDate.getDay()).slice(-2) + ('0' + currentDate.getHours()).slice(-2) +
            ('0' + currentDate.getMinutes()).slice(-2) + '_' + dbFileName;
        setTimeout(() => {
